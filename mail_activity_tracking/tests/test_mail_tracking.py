...
from odoo.addons.mail_activity_tracking.controllers.main import BLANK, MailTrackingController
...
class TestMailTracking(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        for _ in http._generate_routing_rules(
            ["mail", "mail_activity_tracking"], nodb_only=False
        ):
            pass
    ...
    def test_empty_email(self):
        ...
        self.assertEqual(False, self.recipient.email_bounced)
        self.assertEqual(False, self.env["mail.activity.tracking"].email_is_bounced(False))
        self.assertEqual(
            0.0, self.env["mail.activity.tracking"].email_score_from_email(False)
        )
    ...
    @mute_logger("odoo.addons.mail_activity_tracking.controllers.main")
    ...
    def test_mail_tracking_open(self):
        ...
        with patch("odoo.http.db_filter") as mock_client:
            ...
            with patch(
                "odoo.addons.mail_activity_tracking.models.mail_activity_tracking.MailTrackingEmail.search",
    ...
    def test_recordset_email_score(self):
        ...
        trackings = self.env["mail.activity.tracking"]
        for _i in range(11):
            mail, tracking = self.mail_send(self.recipient.email)
            tracking.event_create("click", {})
            trackings |= tracking
    ...
    def test_tracking_img_tag(self):
        ...
        with patch(mock_send_email) as mock_func:
            ...
            self.env["ir.config_parameter"].set_param(
                "mail_activity_tracking.tracking_img_disabled", False
            )
            mail, tracking = self.mail_send(self.recipient.email)
            self.assertEqual(
            ...
            # now we change the system parameter "mail_activity_tracking.img.disable"
            # to True and check that the img tag is not in the outgoing mail
            self.env["ir.config_parameter"].set_param(
                "mail_activity_tracking.tracking_img_disabled", True
            )
            mail, tracking = self.mail_send(self.recipient.email)
            self.assertEqual(
